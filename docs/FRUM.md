...
- **Trust-on-first-use key pinning.** The first public key received from a peer is pinned and presented as a SHA-256 fingerprint for out-of-band comparison. This mirrors the raw-public-key trust model standardized for TLS in RFC 7250 [6] and used in production by SSH for over two decades [7]. Because studies of SSH show that fingerprint verification is the human bottleneck of such schemes — it is rarely performed in the wild [8] — FRUM makes the comparison an explicit step in the UI rather than an optional one.
...
|---|---|---|---|
| UI framework | React [12] | Vue, Svelte, vanilla JS | Largest ecosystem and documentation base; hooks map naturally onto asynchronous browser APIs |
| HTTP framework | Express | Fastify, Koa, bare Node `http` | De facto standard with a large middleware ecosystem; minimal learning curve for a first Node project |
| Runtime | Node.js [10] | Deno, Bun, Python | Event-driven, non-blocking I/O suits long-lived WebSocket connections; one language across client and server |
| Realtime transport | WebSockets [11] | HTTP polling, Server-Sent Events | Bidirectional, persistent single connection, browser-native, no per-message handshake overhead |
| WebSocket server | ws [13] | Socket.io | Minimal relay matches the threat model; Socket.io adds rooms, fallbacks, and reconnection logic that are not needed |
| Database | MongoDB [14] | PostgreSQL, SQLite | Document model matches the metadata records; no relational joins required; managed Atlas hosting |
...
| Cryptography | Web Crypto API [2] | crypto-js, OpenPGP.js, libsodium-wrappers | Native, audited, constant-time implementations; no third-party crypto code in the supply chain |
...
| Build tooling | Vite | Create React App, webpack | Fast dev server and bundling; Create React App is deprecated |
...
| Testing | Vitest, Supertest, Playwright | Jest, Cypress | Fast unit and known-answer tests, HTTP API tests, and browser end-to-end tests |

The browser's native Web Crypto API is chosen over JavaScript cryptography libraries because the implementation's security is the product's entire value. Libraries such as crypto-js are pure JavaScript, which makes them slower and harder to audit than the browser's native, constant-time implementations; OpenPGP.js would drag in an entire PGP stack that the application does not need; and any third-party crypto code adds supply-chain risk to the one component where a bug is catastrophic. The API's algorithm support also constrains the curve choice: ECDH over P-256 is uniformly supported [15], whereas X25519 support is not, so P-256 is the safe default.
